...
if(isset($_POST['idEdit']) || isset($_POST['idE'])){
	...
	if(isset($_POST['idE'])){
		$_POST['idEdit'] = $_POST['idE'];
	}
	
	$idFeuille = $_POST['idEdit'];
	...
	if($valeurs !== false && !empty($valeurs)){
		
		//check droits
		if(checkDroits($valeurs['visibility']) && ($valeurs['droits']==ID || checkDroits("bureau"))){
			
			if($valeurs['idGroupe'] != -1){
				define('TITRE_PAGE',"Editer la feuille de présence");
				$select = "";

			}else{
				define('TITRE_PAGE',"Editer le groupe de feuilles");
				$select= "selectType();";
			}

			$bits = $valeurs['affiche'];
			$types = array();
...
if(isset($_POST['nom'])){
	...
	if(empty($pageMessages)){
		...
		if($valeurs['idGroupe']==-1){
			$res = "Le groupe a bien été modifié.";
		}else{
			$res = "La feuille a bien été modifiée.";
		}
		...
		$editPresence = db_exec($bd, "
					UPDATE membres_presence_feuilles
					SET nom= '".$_POST['nom']."', droits= '".$_POST['droits']."', visibility='".$_POST['droitsView']."' , affiche='".$_POST['membresAffiche']."' , choixRep='".$_POST['choix']."'
					WHERE id='".$_POST['idE']."'");
					
		if($valeurs['idGroupe']==-1){
			$editFeuillesGR = db_exec($bd, "
						UPDATE membres_presence_feuilles
						SET droits= '".$_POST['droits']."', visibility='".$_POST['droitsView']."' , affiche='".$_POST['membresAffiche']."' , choixRep='".$_POST['choix']."'
						WHERE idGroupe='".$_POST['idE']."'");
		...
		if($editPresence !== false && $editFeuillesGR !== false){

			array_push($_SESSION['postMessages'], array('type'=>'ok', 'content'=>$res));
			
			if($valeurs['idGroupe']!=-1){
				header('Location: http://'.$_SERVER['HTTP_HOST'].'/presence-'.$_POST['idE']);
			}else{
...
<br/>
<form method=post action="editPresence.php" id="formEditPresence">


<label id="labelNom" for="nom">nom de la feuille</label>
<input type="text" id="nom" name="nom" style="width:520px" maxlength=150 autocomplete="off" value="<?php echo $valeurs['nom'];?>"/><br/><br/>


<label id="labelFonctionnement" for="droits">fonctionnement de la feuille</label>
	<input id="droitsS" type="radio" name="droits" value="self" <?php echo (($valeurs['droits']=="self")?"checked":"") ?>>  
	<label class="radio" for="droitsS">Une personne ne peut inscrire qu'elle même</label><br/> 
	<input id="droitsM" type="radio" name="droits" value="<?php echo ID; ?>"  <?php echo (($valeurs['droits']==ID)?"checked":"") ?> >  
	<label class="radio" for="droitsM">Vous êtes la seule personne à pouvoir inscire les autres</label><br/> 
	<input id="droitsB" type="radio" name="droits" value="bureau" <?php echo (($valeurs['droits']=="bureau")?"checked":"") ?>>  
	<label class="radio" for="droitsB">Seuls les membres du bureau peuvent inscrire les personnes</label><br/> 
	<input id="droitsA" type="radio" name="droits" value="all" <?php echo (($valeurs['droits']=="all")?"checked":"") ?>>  
...
<br/><br/>
	...
	<input type="checkbox" id="visibleP" name="visibleP" onchange="selectVisible()" <?php echo (($valeurs['visibility']=="probatoire")?"checked":"");?>>
	<label class="checkbox" for="visibleP" style="margin-bottom:10px">Membres en période probatoire</label>
	
	<input type="checkbox" id="visibleM" name="visibleM" onchange="selectVisible()" <?php echo (($valeurs['visibility']=="membre" || $valeurs['visibility']=="probatoire")?"checked":"");?>>
...
<br/><br/>
	...
	<input type="checkbox" id="afficheP" name="afficheP" <?php echo ((in_array("probatoire", $types))?"checked":"") ?>>
	<label class="checkbox" for="afficheP" style="margin-bottom:10px">Membres en période probatoire</label>
	
	<input type="checkbox" id="afficheM" name="afficheM" <?php echo ((in_array("membre", $types))?"checked":"") ?>>
	<label class="checkbox" for="afficheM" style="margin-bottom:10px">Membres actifs</label>
	
	<input type="checkbox" id="afficheB" name="afficheB" <?php echo ((in_array("bureau", $types))?"checked":"") ?>>
...
<label id="labelChoixRep" for="choix">réponses possibles</label>
	<input id="choixON" type="radio" name="choix" value="ON" <?php echo (($valeurs['choixRep']=="ON")?"checked":"")?>>  
	<label class="radio" for="choixON">Oui / Non</label>
	<input id="choixONP" type="radio" name="choix" value="ONP" <?php echo (($valeurs['choixRep']=="ONP")?"checked":"")?>>  
	<label class="radio" for="choixONP">Oui / Non / Peut-être</label>
	<input id="choixONR" type="radio" name="choix" value="ONR" <?php echo (($valeurs['choixRep']=="ONR")?"checked":"")?>>  
	<label class="radio" for="choixONR">Oui / Non / En retard</label> 
	<input id="choixONPR" type="radio" name="choix" value="ONPR" <?php echo (($valeurs['choixRep']=="ONPR")?"checked":"")?>>  
	<label class="radio" for="choixONPR">Oui / Non / Peut-être / En retard</label> 

	
<br/><br/>
<input type="button" onclick="submEditPresence()" id="submitEditPresence" value="valider" />
...
function submEditPresence(){
	...
	if(document.getElementById('nom').value == ""){
		...
		document.getElementById('idE').value = <?php echo $_POST['idEdit']; ?>;
		
		
		document.getElementById('submitEditPresence').disabled=true;
		document.getElementById('submitEditPresence').value = "Patientez...";
		document.getElementById('submitEditPresence').onclick="";
...
<?php
	...
	array_push($_SESSION['postMessages'],array("err", "Impossible de récupérer les informations."));
	header('Location: http://'.$_SERVER['HTTP_HOST'].'/membres/presence.php');
	die();
